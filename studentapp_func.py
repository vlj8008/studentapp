...
#Purpose: Student Tracking App Demo. Demonstrating OOP, Tkinter GUI module,
...
def create_db(self):
    conn = sqlite3.connect('db_studentapp.db')
    with conn:
        cur = conn.cursor()
        cur.execute("CREATE TABLE if not exists tbl_studentapp( \
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_studentapp (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Computer'))
            conn.commit()
    conn.close()

def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_studentapp""")
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email, col_course FROM tbl_studentapp WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course)>0) and ("@" in var_email) and ("." in var_email): 
        conn = sqlite3.connect('db_studentapp.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_studentapp WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_studentapp (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email, var_course))
        ...
        messagebox.showerror("Error", "Something went wrong, please check over all the data you entered to make sure it is correct.")
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_studentapp WHERE col_fullname = '{}'""".format(var_select))
...
def onRefresh(self):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT(*) FROM tbl_studentapp""")
        count = cursor.fetchone()[0]
        i = 0
        while i < count:
                cursor.execute("""SELECT col_fullname FROM tbl_studentapp""")
